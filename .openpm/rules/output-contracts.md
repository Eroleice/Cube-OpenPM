...
`## Coding Report 证据要求` 必须是 SOW 的独立二级标题，不得放在 `## 必跑检查` 下作为编号小节。该章节必须逐条说明后续 coding report 要交付的证据，包括验收项对应源码或测试位置、必跑命令结果、package.json/依赖/脚本状态、未跟踪文件和 OpenPM progression artifact 处理方式。
...
- 不自动拆分或渲染逐条 SOW 验收映射；audit 必须以来源 SOW 原文为准，结合文件清单、Git 审计证据、命令结果和源码事实核验。
...
`code -> audit` 阶段存在受控证据升级例外：如果门禁首次因上下文截断、证据不足、需要全文、缺少源码/测试/配置内容或无法核验而给出 `阻塞` 或 `需要返工`，OpenPM 必须自动补充一次受控全文证据包并重跑该门禁。受控全文证据包只能包含当前 workflow artifact、完整 git status/diff、diff/status 中列出的源码、测试和工程配置文本文件；不得包含 `.env`、`.git/`、`node_modules/`、`dist/`、`coverage/`、`.openpm/logs/`、`.openpm/tmp/` 或路径穿越目标。证据升级重试后仍阻塞时，门禁必须指出具体源码事实、测试断言、配置内容、命令结果或 SOW 条目，不得只重复“上下文被截断”。
...
Audit 最终报告不得摘抄 SOW、coding report、git diff、源码、测试、配置、命令输出或门禁上下文原文；`## 验收证据` / `## 证据` 只能保留来源 artifact 路径、相关文件路径、Git 命令名、章节名和“如需查阅可自行调取”的说明。需要引用具体问题时，写文件路径、命令名和简短事实判断，不复制大段原文。
...
- 相关文件路径: <src/test/config paths>
- 查阅说明: 最终 audit 只记录路径和判断，不摘抄原文；如需复核完整内容，请打开上述路径或重新运行对应命令。
